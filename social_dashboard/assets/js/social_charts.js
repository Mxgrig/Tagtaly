...
function createDualChartCard(containerId, primaryFile, alternateFile, initFunction) {
    ...
    }).catch(error => console.error(`Dual Chart Error for ${containerId}:`, error));
}
...
function initEmotionalRollercoaster() {
    ...
    const initFunc = (content, myChart) => {
        document.getElementById('rollercoaster-title').textContent = content.headline;

        const option = {
            tooltip: { trigger: 'axis' },
            grid: { left: '3%', right: '10%', bottom: '3%', top: '15%', containLabel: true },
            xAxis: { type: 'category', data: content.dates, axisLabel: { ...ECHART_TEXT_STYLE } },
        ...
        .then(content => {
            ...
            chartDom.innerHTML = `
                <div class="winner-card">
                    <div class="winner-topic">${content.topic}</div>
                    <div class="winner-count">${content.count}</div>
                    <div class="winner-subtitle">articles</div>
                </div>
            `;
        }).catch(error => console.error('Weekly Winner Error:', error));
}
...
function initSurgeAlert() {
    ...
    const initFunc = (content, myChart) => {
        document.getElementById('surge-alert-title').textContent = content.headline;
        ...
        const option = {
            ...
            yAxis: {
                type: 'category',
                data: content.data.map(item => item[0]).reverse(),
                axisLabel: { ...ECHART_TEXT_STYLE, fontWeight: 'bold' }
            },
            series: [{
                name: 'Surge Score',
                type: 'bar',
                data: content.data.map(item => item[1]).reverse(),
...
function initMediaDivide() {
    ...
    const initFunc = (content, myChart) => {
        document.getElementById('media-divide-title').textContent = content.headline;

        const allOutlets = [...content.positive_outlets, ...content.negative_outlets];
...
function initSentimentShowdown() {
    ...
    const initFunc = (content, myChart) => {
        document.getElementById('sentiment-showdown-title').textContent = content.headline;

        const pos = content.positive;
        const neg = content.negative;
